...
class BrazoWindow(Window):
    ...
    def on_load_button_clicked(self, widget, data=None):
        ...
        if active_filename != -1:
            active_filename = self.ui.file_selector_combo_box.get_model()[active_filename][0]
    ...
    def load_arm_descriptor(self, filename):
        ...
        while(line):
            ...
            if line.startswith("segment"):
                segment_number = int(line[7]) # We'll parse out the number of the segment.
                if segment_number > 6:
                    self.set_status("This program does not support more than 6 segments.")
                elif segment_number >= len(lengths):
                    self.set_status("Found more segments than declared!")
                else:
                    lengths[segment_number] = int(line[9]) # Parse out the arm length
        return lengths
